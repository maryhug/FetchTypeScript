...
export default function FruitsPanel() {
  ...
  async function handleSearch(e: React.FormEvent) {
    ...
    if (!isNaN(byId)) {
      setLoading(true);
      const result = await fruitsService.getOne(byId);
      if (result.data) {
        setFruits([result.data]);
        setSource(result.source);
      } else {
        notify(`❌ Fruta con ID ${byId} no encontrada`);
      }
      setLoading(false);
    } else {
      await handleGetAll();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (editingId !== null) {
      ...
      notify(result.data ? `✅ Fruta #${editingId} actualizada` : `❌ ${String(result.error)}`);
    } else {
      result = await fruitsService.create(body);
      notify(result.data ? `✅ Fruta "${result.data.name}" creada con ID ${result.data.id}` : `❌ ${String(result.error)}`);
    }
  ...
  async function handleDelete(id: number) {
    ...
    notify(result.error ? `❌ ${String(result.error)}` : `🗑️ Fruta #${id} eliminada`);
    await handleGetAll();
  }
  ...
  return (
    <div className="panel">
      ...
      <form onSubmit={handleSearch} className="search-bar">
        <input
          placeholder="Buscar por ID (ej: 3) o nombre (ej: Mango)"
          value={searchQuery}
      ...
      {fruits.length > 0 && (
        <div className="fruits-grid">
          {fruits.map((f, idx) => (
            <div key={`fruit-${f.id}-${idx}`} className="fruit-card">
              <div className="fruit-name">
                {f.name} <span className="fruit-id">#{f.id}</span>
              </div>
